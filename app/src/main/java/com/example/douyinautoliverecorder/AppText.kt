...
object AppText {
    ...
    fun addRoom(context: Context) = if (zh(context)) "\u6dfb\u52a0\u76f4\u64ad\u95f4" else "Add Room"
    ...
    fun topOverlayLabel(context: Context) = if (zh(context)) "\u9876\u90e8\u5f55\u5236\u5934\u50cf\u6761" else "Top recording avatar bar"
    fun topOverlayEnabled(context: Context) = if (zh(context)) "\u5f55\u5236\u65f6\u663e\u793a\u5728\u5c4f\u5e55\u6700\u4e0a\u65b9" else "Show at the top of the screen while recording"
    fun topOverlayHint(context: Context) = if (zh(context)) "\u8981\u5728\u5c4f\u5e55\u6700\u4e0a\u65b9\u663e\u793a\u5934\u50cf\u6761\uff0c\u8bf7\u5148\u6388\u4e88\u60ac\u6d6e\u7a97\u6743\u9650" else "Grant overlay permission to pin the avatar bar at the top of the screen"
    fun topOverlayReady(context: Context) = if (zh(context)) "\u5f55\u5236\u65f6\u4f1a\u5728\u5c4f\u5e55\u9876\u90e8\u663e\u793a\u5934\u50cf\u6761" else "The avatar bar will appear at the top of the screen while recording"
    fun grantOverlayPermission(context: Context) = if (zh(context)) "\u6388\u4e88\u60ac\u6d6e\u7a97\u6743\u9650" else "Grant overlay permission"
    ...
    fun lastLiveStart(context: Context, time: CharSequence) = if (zh(context)) "\u6700\u540e\u5f00\u64ad: $time" else "Last live start: $time"
    fun lastLiveEnd(context: Context, time: CharSequence) = if (zh(context)) "\u6700\u540e\u4e0b\u64ad: $time" else "Last live end: $time"
    fun liveStartedAt(context: Context, time: CharSequence) = if (zh(context)) "\u5f00\u64ad $time" else "Live at $time"
    ...
    fun statusLabel(context: Context, status: RoomStatus): String = when (status) {
        ...
        RoomStatus.ERROR -> "ERROR"
    }
    fun statusPrefix(context: Context, status: RoomStatus) = if (zh(context)) "\u72b6\u6001: ${statusLabel(context, status)}" else "Status: ${statusLabel(context, status)}"
    ...
    fun reloadPage(context: Context) = if (zh(context)) "\u5237\u65b0\u9875\u9762" else "Reload Page"
    ...
    fun recordingStarted(context: Context) = if (zh(context)) "\u5f00\u59cb\u5f55\u5236" else "Recording started"
